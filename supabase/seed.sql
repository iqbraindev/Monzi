...
INSERT INTO pack_limits (pack_id, max_workspaces, max_agents, max_subaccounts, ai_messages_per_month, ai_messages_per_day, max_dashboards, max_widgets_per_dashboard, max_integrations, voice_enabled, custom_avatar_enabled, storage_mb, support_level)
SELECT id, 1,  1,  0,  50,    10,  1, 5,  3,  false, false, 100,  'community' FROM packs WHERE slug = 'free'
UNION ALL
SELECT id, 2,  3,  2,  500,   50,  3, 15, 5,  true,  false, 1000, 'email'     FROM packs WHERE slug = 'starter'
UNION ALL
SELECT id, 5,  10, 5,  2000,  200, 10, 30, 20, true,  true,  10000,'chat'     FROM packs WHERE slug = 'pro'
UNION ALL
SELECT id, -1, -1, 20, -1,    -1,  -1, -1, -1, true,  true, 100000,'dedicated' FROM packs WHERE slug = 'business';
